...
This command allows users of the AddressBook application to add optional remarks to people in their ClassMonitor and edit it if required. The command should have the following format:
...
/**
 * Changes the remark of an existing student in ClassMonitor.
 */
...
:exclamation: You must delete AddressBook’s storage file located at `/data/.json` before running it! Not doing so will cause AddressBook to default to an empty ClassMonitor!
